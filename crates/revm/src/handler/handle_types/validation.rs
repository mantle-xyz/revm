...
pub type ValidateInitialTxGasHandle<'a, EXT, DB> = Arc<
    dyn Fn(&mut Context<EXT, DB>) -> Result<InitialAndFloorGas, EVMError<<DB as Database>::Error>>
        + 'a,
>;
...
pub struct ValidationHandler<'a, EXT, DB: Database> {
    ...
    pub initial_tx_gas: ValidateInitialTxGasHandle<'a, EXT, DB>,
...
impl<'a, EXT: 'a, DB: Database + 'a> ValidationHandler<'a, EXT, DB> {
    ...
    pub fn new<SPEC: Spec + 'a>() -> Self {
        Self {
            initial_tx_gas: Arc::new(mainnet::validate_initial_tx_gas::<SPEC, EXT, DB>),
