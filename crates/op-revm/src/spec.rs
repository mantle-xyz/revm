...
impl OpSpecId {
    ...
    pub const fn into_eth_spec(self) -> SpecId {
        match self {
            ...
            // Interop (Lagoon) is newer than Karst, so its eth base must not be older than
            // Karst's (OSAKA). [MANTLE] OSAKA (Limb) / ARSIA are Mantle-only Osaka-level specs
            // and share the same eth base. Note: on Mantle chains INTEROP is not selected
            // (env.rs resolves Mantle forks to ISTHMUS/OSAKA/ARSIA), so this only matters for
            // upstream parity.
            Self::KARST | Self::INTEROP | Self::OSAKA | Self::ARSIA => SpecId::OSAKA,
        }
    }
