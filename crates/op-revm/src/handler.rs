    ...
    fn validate_initial_tx_gas(
        ...
        if tx.tx_type() == DEPOSIT_TRANSACTION_TYPE {
            ...
            if !cfg.spec().is_enabled_in(OpSpecId::ARSIA) {
                ...
                // Pre-ARSIA has no EIP-8037 state gas, so the intrinsic gas lives entirely in
                // `initial_regular_gas`; scaling it scales the total intrinsic gas.
                initial_gas.initial_regular_gas = initial_gas
                    .initial_regular_gas
