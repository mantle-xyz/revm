    ...
    FRAME: FrameTr<FrameResult = FrameResult, FrameInit = FrameInit>,
{
    ...
    fn refund(
        ...
        // BVM_ETH access-cost alignment with op-geth is now handled at the
        // source: BvmEth::process_eth_deposit (and mint/transfer) cools the
        // BVM_ETH account and touched storage slots after pre-EVM journal
        // mutations, so subsequent EVM access pays cold cost naturally. The
        // prior flat 4500-gas compensation here over-charged when EVM never
        // accessed BVM_ETH (e.g. EOA target).
