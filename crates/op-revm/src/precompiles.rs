...
impl OpPrecompiles {
    ...
    pub fn new_with_spec(spec: OpSpecId) -> Self {
        let precompiles = match spec {
            // [MANTLE] OSAKA/ARSIA derive their precompiles directly from the mapped eth
            // spec (Osaka). This preserves the op-revm 19 behavior; we intentionally do NOT
            // route them through the new upstream karst() set in this upgrade.
            ...
            // [MANTLE] Keep INTEROP/KARST aligned with upstream (karst precompiles). OSAKA
            // (Limb) / ARSIA are Mantle-only and derive their precompiles from the mapped eth
            // spec in the arm above; this upgrade intentionally does not re-route them.
