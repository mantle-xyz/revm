...
## [13.0.0](https://github.com/mantle-xyz/revm/compare/op-revm-v12.0.2...op-revm-v13.0.0) - 2026-06-11
...
- *(op-revm)* keep BVM_ETH transfer on OOG-halted deposits + correct CREATE recipient
- *(op-revm)* a halted failed deposit keeps only the Mint log (not the transfer)
- *(op-revm)* persist BVM_ETH mint/transfer logs on REVERTED failed deposits
- *(op-revm)* persist BVM_ETH transfer log too on failed deposits
...
- *(op-revm)* cover CREATE+OOG failed deposit (both fixes composed)
- *(op-revm)* assert failed deposit surfaces BVM_ETH mint log (block 96442768)
- *(op-revm)* run deposit gas tests under ARSIA (Osaka) spec to match production
- *(op-revm)* expand cooling coverage — matrix, gas E2E, revert, CREATE, nested
- Audit fix ([#25](https://github.com/mantle-xyz/revm/pull/25))
- Merge pull request #24 from mantle-xyz/arsia-security-fix
